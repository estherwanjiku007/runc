#FROM  command_Used to run containers 
FROM :python 3.12.1
# Run cammand used to run the application
RUN apk add --no-cache python g++ make;

# WorkDir>Usedto emsure that the reqiredffilters
WORKDIR "./application";
# Copy thespurce code locally
COPY ...

RUN["yarn install",""production"];
CMD ["Node js ,supabase,"]
# ARG GO_VERSION=1.23
